...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    const pendingRole = cookieStore.get("google_oauth_role")?.value ?? "PATIENT";
      ...
      if (existingEmailUser) {
        ...
        const newUser = await prisma.user.create({
          data: {
            ...
            name,
            picture,
            role: pendingRole === "COMPANY" ? "COMPANY" : "USER",
    ...
    response.headers.append(
      "Set-Cookie",
      "google_oauth_role=; Max-Age=0; Path=/; HttpOnly; SameSite=lax"
    );
