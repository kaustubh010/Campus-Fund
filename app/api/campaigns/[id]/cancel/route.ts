...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (action === "prepare") {
      ...
      const state = await getAppGlobalState(campaign.appId);
      const totalRaisedMicro = Number(state.total_raised ?? 0);

      const v = validateDonationsForRefundPlan(donations, totalRaisedMicro);
      if (v) return NextResponse.json({ error: v }, { status: 400 });

      const fullPlan = buildRefundPlan(donations, totalRaisedMicro);
