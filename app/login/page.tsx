  ...
  const onboardingData = [
    {
      ...
      description:
        "All campaigns are backed by transparent smart contracts on the Algorand blockchain.",
      icon: ShieldCheck,
      color: "text-[#6EE7B7]",
      bg: "bg-[#6EE7B7]/10",
    ...
    },
    {
      ...
      description:
        "Join forces with students to fund innovative campus projects block by block.",
      icon: Users,
      color: "text-[#818CF8]",
      bg: "bg-[#818CF8]/10",
    ...
    },
    {
      title: "Smart Escrow",
      description:
        "Donations are safely locked in escrow and only release when the campaign goal is achieved.",
      icon: Lock,
      color: "text-[#FCD34D]",
      bg: "bg-[#FCD34D]/10",
  ...
  if (step === "splash") {
    return (
      <div className="min-h-screen bg-[#0A0A0F] flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <p className="text-[#64748B] text-lg font-medium z-10">
    ...
    return (
      <div className="min-h-screen bg-[#0A0A0F] flex flex-col relative overflow-hidden">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center z-10">
          <div
            className={`w-32 h-32 rounded-[2rem] flex items-center justify-center mb-8 border ${current.bg} ${current.border} ${current.color} shadow-2xl`}
          >
            <current.icon className="w-16 h-16" strokeWidth={1.5} />
          </div>
          <h2 className="text-3xl font-bold font-[Syne] text-[#F1F5F9] mb-4">
            {current.title}
          </h2>
          <p className="text-lg text-[#64748B] max-w-sm leading-relaxed">
            {current.description}
          </p>
        </div>
        ...
        <div className="p-8 pb-12 z-10">
          <div className="flex justify-center gap-3 mb-10">
            {onboardingData.map((_, i) => (
              <div
                key={i}
                className={`h-2 rounded-full transition-all duration-500 ${i === onboardingStep ? "w-10 bg-[#6EE7B7]" : "w-3 bg-[#1E1E2E]"}`}
              />
            ))}
          </div>
          <button
          ...
          >
            {onboardingStep < 2 ? "Continue" : "Get Started"}{" "}
            <ArrowRight className="w-5 h-5" />
          </button>
          <button
            onClick={finishOnboarding}
            className="w-full py-4 mt-2 text-[#64748B] hover:text-[#F1F5F9] font-medium text-lg transition-colors"
          >
            Skip
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] flex flex-col p-8 md:p-12 relative overflow-hidden mt-20">
      ...
      <div className="w-full max-w-md mx-auto z-10 hidden md:flex items-center gap-2 mb-8">
        ...
        <span className="text-2xl font-bold text-[#F1F5F9] font-[Syne] tracking-wide group-hover:text-[#6EE7B7] transition-colors">
          CampusFund
        </span>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center max-w-md mx-auto w-full z-10">
        <div className="md:hidden flex items-center gap-2 mb-8">
          ...
          <span className="text-2xl font-bold text-[#F1F5F9] font-[Syne] tracking-wide transition-colors">
            CampusFund
          </span>
        </div>

        <h2 className="text-4xl font-bold font-[Syne] text-[#F1F5F9] mb-2">
          Welcome Back
        </h2>
        ...
        <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <a
                href="#"
                className="text-sm font-bold text-[#6EE7B7] hover:underline"
              >
                Forgot?
              </a>
            </div>
          ...
          <p className="text-center text-[#64748B] mt-6">
            ...
            <Link
              href="/signup"
              className="text-[#6EE7B7] font-bold hover:underline"
            >
              Create one
            </Link>
          </p>
          ...
          <div className="relative py-8">
            ...
            <div className="relative flex justify-center">
              <span className="bg-[#0A0A0F] px-4 text-sm font-bold text-[#64748B]">
                OR
              </span>
            </div>
          </div>
          ...
          <a
            ...
            className="w-full py-4 bg-[#111118] border border-[#1E1E2E] text-[#F1F5F9] text-lg font-bold rounded-xl hover:bg-[#1E1E2E] transition-colors flex items-center justify-center gap-3"
          >
            <svg
